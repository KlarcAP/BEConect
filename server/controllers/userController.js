...
export const loginUser = async (req, res) => {
  try {
    ...
    if (user && isMatch) {
      ...
      return res
        .status(401)
        .json({ status: false, message: "E-mail ou senha inválida" });
    }
  } catch (error) {
    console.log(error);
    return res.status(400).json({ status: false, message: error.message });
...
export const markNotificationRead = async (req, res) => {
  try {
    ...
    res.status(201).json({ status: true, message: "Feito" });
  } catch (error) {
    console.log(error);
    return res.status(400).json({ status: false, message: error.message });
  }
    ...
    if (user) {
      ...
      res.status(201).json({
        ...
        message: `A conta do usuário foi ${
          user?.isActive ? "ativada" : "desabilitada"
        }`,
      });
    } else {
      res.status(404).json({ status: false, message: "Usuário não encontrado" });
...
export const deleteUserProfile = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: true, message: "Usuário excluído com sucesso" });
  } catch (error) {
    console.log(error);
    return res.status(400).json({ status: false, message: error.message });
  }
